...
# nice_bar_plot(hcat(firstpass, Dfirstpass), xlims=(0, 15.5), xlabel="# Draws to decision", labels=["Full Bayesian model" "Commitment model λ = 0.15" "Commitment model λ = 0.25"], ylims=(0, 0.2), colpal= PTolsGrad2, bar_width=0.7, legendfontsize=14)
...
##
idx = findall(0.1 .<= lambdas .<= 0.25) #0.0-0.15, 0.1-0.25
...
##
M = [mean(discounted[0.0 .<= lambdas .<= 0.15]), mean(discounted[0.1 .<= lambdas .<= 0.25])]
bar(["0.0 < λ 0.15", "0.1 < λ 0.25"],  M, ylabel="Asymetric evidence integration\n(% observations)", label="", color = [PTols[3], PTols[2]], linewidth=0.0, yticks=0.0:0.1:0.9, ylims=(0,0.57), labelfontsize=18, tickfontsize=14, size=(500, 500), dpi=300, background_color=:transparent)
